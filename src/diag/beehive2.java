...
public class beehive2 {
	...
	static String logsLoc = "D:\\git\\email1";	
	...
	public static void main(String[] args) {
		...
		ArrayList<File> diagLogFiles = new ArrayList<File>();			
		listf(logsLoc+"\\"+timeStamp, diagLogFiles);
		
		
		String line = null;
		for (File fileName : diagLogFiles) {
			try {
	...
	public static void dumpPart(Part p) throws Exception {
		...
		if (saveAttachments && level != 0 && p instanceof MimeBodyPart && !p.isMimeType("multipart/*")) {
			...
			if (disp == null || disp.equalsIgnoreCase(Part.ATTACHMENT)) {
				...
				filename = logsLoc+"\\"+timeStamp+"\\"+messageId+"\\"+filename;
				...
				if (filename.contains("zip")) {
				    try {
				         ...
				         zipFile.extractAll(logsLoc+"\\"+timeStamp+"\\"+messageId);
	...
	public static void listf(String directoryName, ArrayList<File> files) {
	    ...
	    for (File file : fList) {
	        if (file.isFile() &&
	        	file.getName().equals("Diagnostics.log")) {
	            files.add(file);
